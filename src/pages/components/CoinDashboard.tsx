...
export default function CoinDashboard({ isDark }: { isDark: boolean }) {
  const canvasElCollateral = React.useRef();
  const canvasElBorrowing = React.useRef();

  React.useEffect(() => {
    const ctx = canvasElCollateral.current?.getContext("2d");
    const borrowCtx = canvasElBorrowing.current?.getContext("2d");
    ...
    const earnAPR = [
      3.85, 3.39, 3.5, 4.92, 3.11, 2.47, 4.04, 3.49, 4.58, 4.35, 3.15, 3.07,
      3.13, 4.51, 4.84, 3.18, 4.78, 2.96, 4.53, 3.62, 4.34, 2.59, 3.55, 4.85,
      3.12, 4.34, 4.24, 3.21, 2.83, 2.25,
    ];
    ...
    <div className="m-5 mx-20 flex flex-col p-10">
      ...
      <div className="mb-2 mt-10 flex justify-between text-white">
        ...
        <div className="flex w-[45%] flex-col">
          ...
          <div>
            <canvas id="BorrowingGraph" ref={canvasElBorrowing} height="100" />
          </div>
        </div>
      </div>
    </div>
